...
첫 화면의 기사 영역은 각 연구 항목의 `article` 데이터를 사용한다. `Research Dispatch`를 선택하면 Cover Story 헤드라인, 리드, `In This Dispatch` 본문, 세 개의 기사 카드가 선택된 연구 주제에 맞게 전환된다. 전환 순간에는 신문지가 넘어가는 느낌의 paper sheet 애니메이션을 얹고, 의미 없는 페이지 번호 표기는 사용하지 않는다.
